    ...
    if (ship_->visible_)
    {
        ...
        {   Color3f color(1.f, 0.f, 0.f);
            ...
            if (ship_->docked_)
                text::drawSpaceText(ship_->owner_->name(),
                    ship_->location_ + Vector2f(0.f, -ship_->radius_*2.5f), 12.f, TEXT_ALIGN_CENTER, color);
            else
        ...
        if (ship_->fragStars_ > 0)
        {
            ...
            if (ship_->docked_)
                ...
                ship_->location_ + Vector2f(0.f, -ship_->radius_*2.5f) + Vector2f(0.f, -17.f), 25.f, TEXT_ALIGN_CENTER, color);
            else
