...
void LanguageButton::draw() const
{
    ...
    Color3f color(getColor3f(0.7f, 0.7f, 0.7f)*(1-highlight) +
        highlight*(getColor3f(1.f, 0.6f, 0.8f)*(1-hoveredFadeTime_) +
        getColor3f(1, 1, 1)*hoveredFadeTime_));

    text::drawFooText();
    text::drawScreenText(locales::getCurrentLocale().name_,
        origin + Vector2f((width()+labelWidth_*mirror)*0.5f,1), 12.f, TEXT_ALIGN_CENTER, color);
