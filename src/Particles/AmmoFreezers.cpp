...
AmmoFreezers::AmmoFreezers(Vector2f const& location, Vector2f const& direction, Vector2f const& velocity, Color3f const& color, Player* damageSource)
    :Particle<AmmoFreezers>(spaceObjects::oAmmoFreezers, location, randomizer::random(9.f, 15.f), 0.01f, randomizer::random(12.f, 15.f))
{
...
void AmmoFreezers::update()
{
    ...
    if (lifeTime_ > totalLifeTime_)
    {
        ...
        //     particles::spawn(particles::pMiniAmmoFreezers, location_, Vector2f(), Vector2f(), Color3f(), damageSource_);
    }
}
...
void AmmoFreezers::shockWave(Vector2f const& location, float strength, float radius)
{
