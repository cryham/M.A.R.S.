...
void Lightning::fire() const
{
    ...
    particles::spawn(particles::pAmmoLightning, parent_->location() + faceDirection*parent_->radius(), faceDirection, parent_->velocity(), Color3f(), parent_->getOwner());
...
float Lightning::maxAngle() const
{
    return 70.f;
}
