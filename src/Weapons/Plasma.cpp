...
void Plasma::draw(float alpha) const
{
    ...
    const int u = 0, v = 29;
    glBegin(GL_QUADS);
        uv8w(u, v);     glVertex2f(0,     r* 0.6f);
        uv8w(u, v+1);   glVertex2f(0,     r*-0.6f);
        uv8w(u+1, v+1); glVertex2f(r*5.f, r*-0.6f);
        uv8w(u+1, v);   glVertex2f(r*5.f, r* 0.6f);
    glEnd();
}
...
void Plasma::fire() const
{
    ...
    particles::spawn(particles::pAmmoPlasma, parent_->getLocation() + dir*parent_->getRadius(),
