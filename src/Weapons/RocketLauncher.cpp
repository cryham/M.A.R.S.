...
void RocketLauncher::draw(float alpha) const
{
    ...
    glBegin(GL_QUADS);
        ...
        glTexCoord2f((u+1)*0.125f,(v+1)*0.03125f); glVertex2f(r*2.f, r*-0.5f);
        glTexCoord2f((u+1)*0.125f, v*0.03125f);    glVertex2f(r*2.f, r* 0.5f);
    glEnd();
}
...
void RocketLauncher::fire() const
{
    ...
    {   timer_ = time;
        ...
        particles::spawn(particles::pAmmoRocket, parent_->location() + dir*parent_->radius(), dir,
        ...
        parent_->velocity() -= dir * 400.f;  // knock back
